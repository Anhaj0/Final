...
import { AssignmentsModule, ComplaintsModule, LostFoundModule, RoutesStopsModule, getAdminModuleIcon } from './adminModules';
...
const DriversModule = () => {
  ...
  const fetchDrivers = async () => {
    try {
      const res = await axios.get(`${API_BASE}/api/admin/driver-profiles`);
      setDrivers(res.data);
    } catch { setDrivers([]); }
    setLoading(false);
  };
  ...
  const saveDriver = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE}/api/admin/driver-profiles/user/${editingId}`, {
      ...
      setMsg(err.response?.data?.message || 'Failed to save driver');
    }
  };
  ...
  const deleteDriver = async (driverUserId) => {
    if (!window.confirm('Delete this driver account?')) return;
    setMsg('');
    try {
      await axios.delete(`${API_BASE}/api/admin/driver-profiles/user/${driverUserId}`);
      setMsg('Driver deleted successfully');
      if (editingId === driverUserId) {
        setEditingId(null);
        setForm({ fullName: '', email: '', phoneNumber: '', password: '' });
        setShowForm(false);
      }
      ...
      setMsg(err.response?.data?.message || 'Failed to delete driver');
    }
  };
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6">
          <h3 className="font-semibold text-slate-800 dark:text-white mb-4">{editingId ? 'Edit Driver Account' : 'Create Driver Account'}</h3>
          <form onSubmit={saveDriver} className="grid grid-cols-2 gap-4">
            ...
            <input value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder={editingId ? 'Password (leave blank to keep current)' : 'Password'} type="password" required={!editingId} className="px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all" />
            <button type="submit" className="col-span-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition-colors">{editingId ? 'Update Driver' : 'Create Driver'}</button>
          </form>
        </div>
      )}
        ...
        drivers.length === 0 ? <div className="p-8 text-center text-slate-400 dark:text-slate-500">No drivers registered. Use "Add Driver" to create one.</div> : (
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              <tr>
                <th className="px-6 py-4">Name</th><th className="px-6 py-4">Email</th><th className="px-6 py-4">Phone</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
              {drivers.map(d => (
                <tr key={d.id || d.userId} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-3">
                      <button type="button" onClick={() => editDriver(d)} className="text-blue-600 hover:text-blue-800 font-medium transition-colors">Edit</button>
                      <button type="button" onClick={() => deleteDriver(d.userId)} className="text-rose-600 hover:text-rose-800 font-medium transition-colors">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
...
const BusManagement = () => {
  ...
  const deleteBus = async (busId) => {
    ...
    setGenMsg('');
    try {
      await axios.delete(`${API_BASE}/api/buses/${busId}`);
      if (editingId === busId) {
        setEditingId(null);
        setForm(emptyBus);
      }
          ...
          {buses.map(bus => (
            <div key={bus.id} className="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6 transition-transform hover:-translate-y-1 hover:shadow-xl duration-300">
              ...
              <div className="flex gap-3 mb-4">
                <button type="button" onClick={() => editBus(bus)} className="text-blue-600 hover:text-blue-800 font-medium transition-colors">Edit</button>
                <button type="button" onClick={() => deleteBus(bus.id)} className="text-rose-600 hover:text-rose-800 font-medium transition-colors">Delete</button>
              </div>
...
const PlaceholderModule = ({ tabName }) => {
  ...
  const Icon = getAdminModuleIcon(tabName) || iconMap[tabName] || LayoutDashboard;
  return (
...
export default function App() {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'Routes & Stops') return <RoutesStopsModule />;
    if (activeTab === 'Lost & Found') return <LostFoundModule />;
    if (activeTab === 'Complaints') return <ComplaintsModule />;
    ...
    <div className="h-screen bg-slate-50 dark:bg-gray-950 text-slate-800 dark:text-slate-200 flex font-sans overflow-hidden selection:bg-blue-500/30 transition-colors duration-300">
      <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800/80 flex flex-col h-full flex-shrink-0 relative z-20 transition-colors duration-300">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          ...
          <SidebarItem icon={ShieldCheck} label="Assignments" active={activeTab === 'Assignments'} onClick={() => setActiveTab('Assignments')} />
